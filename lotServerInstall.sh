...
function Install() {
  ...
  URLKernel='https://github.com/acgpiano/lotServer/raw/master/lotServer.log'
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://github.com/acgpiano/lotServer/raw/master/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://github.com/acgpiano/lotServer/raw/master/lotServer.tar"
  tar -xvf "/tmp/lotServer.tar" -C /tmp
  acce_ver=$(acce_check ${KNV})
  # 如果有自己搭建的或者api失效，这里修改成你自己的api
  wget --no-check-certificate -qO "${AcceTmp}/etc/apx.lic" "https://apx.0025.top/keygen.php?ver=1&mac=52:54:00:37:d3:8a"
